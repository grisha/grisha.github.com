---
layout: post
title: "State of Tgres - Summer 2016"
date: 2016-07-29 10:12
comments: true
categories:
---

It's been over a year since I began hacking on
[Tgres](http://github.com/tgres/tgres). Tgres came about when I first
decided to dive into [Go](https://golang.org/). I really liked what Go
could do, but didn't see how it applied to anything I was working on
at the time. I needed a project that was a better match for the domain
of applications that Go aims for, something where performance and
scale matter, something with concurrent moving pieces, something
...
So Tgres is a metrics collection and storage server, aka a time series
...
Don't get too excited just yet, Tgres is still not production quality,
though it's probably stable enough for someone who knows their way
around Go to give it a whirl. At this point I am definitely proved the
...
forward from MRTG which was merely a Perl script. RRDTool is to this
day is the best C implementation of a round-robin database for time
series data. Similarly to MRTG, RRDTool was designed as a command-line
tool, the server component was left as an exercise for the user. And
even though linking RRDTool into your app was not too difficult
(I [did it](https://github.com/grisha/openvps-common/blob/master/py-rrd/_RRD.c)
in 2004), somehow an "RRD server" never appeared.

Then there was [Graphite](https://graphiteapp.org/). Graphite is a
reflection of the Python-can-do-anything era. Graphite borrowed a lot
of ideas from RRDTool, but its re-implementation of round-robin
on-disk files while [claiming superiority](http://graphite.wikidot.com/whisper) is actually
[quite inferior](http://grisha.org/blog/2015/05/04/recording-time-series/)
to RRDTool in both accuracy and performance.

Graphite's appeal was that it was an easy-to-start server that did
everything, and it became especially popular along side
...
dub _data isolation_. We are so used to it, we don't even
question it.

Data isolation is when a part of our data is stored using a separate
tool in a different format and is therefore not as easily accessible.
For example if our metrics are in Graphite, we probably don't even
know how to get them out of it, nor does it occur to us that it might
be useful.  All we've been able to do is get a Grafana chart and are
quite satisfied with it. We do not question why it isn't a first-class
citizen right in the database as a table, where we could use it in SQL
joins, for example. Or export it to our big data rig and query it with
Hive or Spark, etc.
...
by the _step_ (resolution) and the size of the archive. Thus RRA's are
...
The[transceiver](https://github.com/tgres/tgres/tree/master/transceiver)
...
[Statd](https://github.com/tgres/tgres/tree/master/statd) is currently
...
There's still a lot of work to be on Tgres. For one thing, I don't
have any tests. This is mainly because I don't believe in testing that
which hasn't "gelled", and I wouldn't be surprised if the above
organization of packages and how they interface changes as I
understand the problem better.

We also need documentation. And some real-life use/testing/feedback
